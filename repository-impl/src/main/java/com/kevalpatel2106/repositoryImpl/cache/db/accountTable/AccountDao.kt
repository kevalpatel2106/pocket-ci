...
internal interface AccountDao {
    ...
    @Query("SELECT COUNT(${AccountTableInfo.ID}) FROM ${AccountTableInfo.TABLE_NAME} WHERE ${AccountTableInfo.BASE_URL} = :url AND ${AccountTableInfo.AUTH_TOKEN} = :token")
    ...
    @Query("UPDATE ${AccountTableInfo.TABLE_NAME} SET ${AccountTableInfo.NEXT_PAGE_CURSOR} = :cursor WHERE ${AccountTableInfo.ID} = :accountId")
    suspend fun updateNextPageCursor(accountId: Long, cursor: String?)

    @Query("SELECT ${AccountTableInfo.LAST_PROJECT_REFRESH} FROM ${AccountTableInfo.TABLE_NAME}  WHERE ${AccountTableInfo.ID} = :accountId LIMIT 1")
    suspend fun getLastProjectRefreshEpoch(accountId: Long): Long

    @Query("UPDATE ${AccountTableInfo.TABLE_NAME} SET ${AccountTableInfo.LAST_PROJECT_REFRESH} = :epoch WHERE ${AccountTableInfo.ID} = :accountId")
