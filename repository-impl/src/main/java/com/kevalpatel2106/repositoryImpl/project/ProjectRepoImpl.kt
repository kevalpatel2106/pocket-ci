...
internal class ProjectRepoImpl @Inject constructor(
    ...
    override suspend fun getProject(remoteId: String, accountId: AccountId): Project? {
        return if (projectDao.getCount(remoteId, accountId.getValue()) > 0) {
            val dao = projectDao.getProject(remoteId, accountId.getValue())
            projectMapper(dao)
        } else {
            null
        }
